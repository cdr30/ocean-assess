 MODULE cdfio
  ...
  !!------------------------------------------------------------------------------------------------------
  !!  $Rev: 250 $
  !!  $Date: 2009-05-26 19:00:35 +0200 (Tue, 26 May 2009) $
  !!  $Id: cdfio.f90 250 2009-05-26 17:00:35Z molines $
  !!--------------------------------------------------------------
  USE netcdf 
...
! REAL(kind=4),  DIMENSION(:,:), ALLOCATABLE :: e3u_ps, e3v_ps !: for reading e3._ps in nemo3.x
  ...
  TYPE, PUBLIC ::   variable 
     ...
     CHARACTER(LEN=300):: short_name
     CHARACTER(LEN=300):: online_operation
     CHARACTER(LEN=300):: axis
     CHARACTER(LEN=300):: PRECISION='r4'  ! possible values are i2, r4, r8
  END TYPE variable
  ...
  PRIVATE 
  PUBLIC  copyatt, copy_time, create, createvar, getvaratt,cvaratt
...
CONTAINS
  ...
  FUNCTION copy_time( icout, cdfileref)
    !!------------------------------------------------------------------
    !! Copy value(s) and attributes of the time variable from cdfileref 
    !! into cdfile
    !!------------------------------------------------------------------
    CHARACTER(LEN=*), INTENT(in) :: cdfileref
    INTEGER,  INTENT(in) :: icout
    INTEGER   :: istatus, ncid, idum, npt
    INTEGER :: copy_time
    REAL(KIND=4), DIMENSION (:),       ALLOCATABLE ::  tim  

    istatus = NF90_OPEN(cdfileref,NF90_NOWRITE,ncid)
    IF (istatus /= 0) CALL ERR_HDL(istatus)

!    istatus = NF90_INQ_VARID(icout,'time_counter',id_tim)
!    IF (istatus /= 0) CALL ERR_HDL(istatus)

    istatus = NF90_REDEF(icout)
    IF (istatus /= 0) CALL ERR_HDL(istatus)

    istatus = copyatt('time_counter',id_tim,ncid,icout)
    IF (istatus /= 0) CALL ERR_HDL(istatus)

    istatus = NF90_ENDDEF(icout)
    IF (istatus /= 0) CALL ERR_HDL(istatus)
    
    npt = getdim (cdfileref,'time_counter')
    ALLOCATE (tim(npt))

    tim = getvar1d(cdfileref,'time_counter',npt)
    istatus=putvar1d(icout,tim,npt,'T')
    IF (istatus /= 0) CALL ERR_HDL(istatus)
  ...
  FUNCTION create( cdfile, cdfilref ,kx,ky,kz ,cdep)
    ...
    CHARACTER(LEN=300) :: cldep, cldepref
  ...
  FUNCTION getvarid( cdfile, knvars )
    ...
    !! * local declarations
    CHARACTER(LEN=300), DIMENSION(knvars) :: cdvar
    INTEGER :: ncid, jv
    INTEGER :: istatus
  ...
  FUNCTION getvaratt (cdfile,cdvar,cdunits, pmissing_value, cdlong_name, cdshort_name)
    ...
    !! ----------------------------------------------------------------------------------------------------
    ! * Arguments
    CHARACTER(LEN=300), INTENT(in) :: cdfile, cdvar
    CHARACTER(LEN=300), INTENT(out) :: cdunits, cdlong_name, cdshort_name
...
!   istatus = NF90_ENDDEF(ncid)
  ...
  FUNCTION cvaratt (cdfile,cdvar,cdunits,pmissing_value, cdlong_name, cdshort_name)
    ...
    ! * Arguments
    CHARACTER(LEN=300), INTENT(in) :: cdfile, cdvar
    CHARACTER(LEN=300), INTENT(in) :: cdunits, cdlong_name, cdshort_name
    INTEGER :: cvaratt
  ...
  FUNCTION  getdim (cdfile,cdim_name,cdtrue,kstatus,ldexact)
    ...
    CHARACTER(LEN=300),OPTIONAL, INTENT(out) ::  cdtrue ! full name of the read dimension
  ...
  FUNCTION getvdim (cdfile, cdvar)
    ...
    INTEGER :: jvar
    CHARACTER(LEN=300) :: clongname='long_name', clongn
  ...
  FUNCTION  getvarname (cdfile, knvars, ptypvar)
    ...
    CHARACTER(LEN=*), INTENT(in) :: cdfile
    INTEGER, INTENT(in)  ::  knvars                  ! Number of variables in cdfile
    CHARACTER(LEN=300), DIMENSION(knvars) :: getvarname
  ...
  FUNCTION  getvar (cdfile,cdvar,klev,kpi,kpj,kimin,kjmin, ktime,ldiom)
    ...
    INTEGER :: ncid, id_var, id_var2, id_dimunlim, nbdim
    INTEGER :: istatus, istatus2, ilev, imin, jmin, itime, ilog, ipiglo, imax
        ...
        ! now try to detect if it is v2 or v3, in v3, e3t_ps exist and is a 2d variable
         ...
         IF ( istatus == NF90_NOERR ) THEN  
           ...
           IF ( .NOT. l_mbathy ) THEN
             ...
             !
             istatus2=NF90_INQ_VARID (ncid,'e3t_1d', id_var2)
             istatus=NF90_INQ_VARID (ncid,'e3t_0', id_var)
             IF ( istatus2 == NF90_NOERR ) THEN
                 istatus2=NF90_GET_VAR(ncid,id_var2,e3t_0, start=(/1,1/), count=(/ik0,1/) )
             ELSE IF ( istatus ==  NF90_NOERR ) THEN
                 istatus=NF90_GET_VAR(ncid,id_var,e3t_0, start=(/1,1/), count=(/ik0,1/) )
             ELSE
               PRINT *, 'Problem reading mesh_zgr.nc v3 (pre-v3.6): no e3t_0 found !' ; STOP
             ENDIF
             !
             istatus2=NF90_INQ_VARID (ncid,'e3w_1d', id_var2)
             istatus=NF90_INQ_VARID (ncid,'e3w_0', id_var)
             IF ( istatus2 ==  NF90_NOERR ) THEN
                istatus2=NF90_GET_VAR(ncid,id_var2,e3w_0, start=(/1,1/), count=(/ik0,1/) )
             ELSE IF ( istatus ==  NF90_NOERR ) THEN
         ...
         ELSE
          ! zgr v2: pre-v3.6
          istatus=NF90_INQ_VARID (ncid,'e3t', id_var)
          IF ( istatus == NF90_NOERR ) THEN
              SELECT CASE ( clvar )
  ...
  FUNCTION  getvare3 (cdfile,cdvar,kk)
    ...
    INTEGER, DIMENSION(4) :: istart, icount
    INTEGER :: ncid, id_var
    INTEGER :: istatus, istatus2
    CHARACTER(LEN=300) :: clvar                      ! local name for cdf var (modified)
    ...
    ! IOIPSL (x_a=y_a=1)               IOM (pre-v3.6)   IOM (v3.6+)
    ...
    istatus2=NF90_INQ_VARID ( ncid,'e3t_1d',id_var)
    istatus=NF90_INQ_VARID ( ncid,'e3t_0',id_var)
  ...
  FUNCTION putheadervar(kout, cdfile, kpi,kpj,kpk, pnavlon, pnavlat ,pdep,cdep)
    ...
    CHARACTER(LEN=300),DIMENSION(jpdep ) :: cldept=(/'deptht ','depthu ','depthv ','depthw ','nav_lev'/)
    CHARACTER(LEN=300) :: cldep
  ...
  FUNCTION closeout(kout)
    ...
    IF ( closeout /= 0) THEN
       print *, TRIM(NF90_STRERROR(closeout))
    END IF
  ...
  SUBROUTINE ERR_HDL(kstatus)
    ...
    IF (kstatus /=  NF90_NOERR ) THEN
       WRITE(0,*)  'ERROR in NETCDF routine, status=',kstatus
       WRITE(0,*) NF90_STRERROR(kstatus)
       STOP 1
    END IF
